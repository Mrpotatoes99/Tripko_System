...
if (isset($_SESSION['user_type_id']) && $_SESSION['user_type_id'] == 1) {
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Forbidden: Super Admin accounts cannot create terminal routes.'
    ]);
    exit();
}
...
if (!$origin_terminal_id || !$destination_terminal_id || empty($type_ids)) {
...
try {
    $stmt = $conn->prepare("INSERT INTO transport_routes (from_terminal_id, to_terminal_id) VALUES (?, ?)");
